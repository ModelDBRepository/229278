function [data_structure] =  ch_hgf_analysis_nu(data_structure)
...
data_structure.behavioral.hgf_nu.est = tapas_fitModel(y2, [y1,inputs],  'tapas_hgf_binary_config_startpoints', 'tapas_condhalluc_obs2_config');
% tapas_hgf_binary_condhalluc_plotTraj(data_structure.behavioral.hgf_nu.est)

%  data_structure.behavioral.hgf.est.traj.muhat(:,2) gives trajectory of
%  plots:  first column x1, second x2, third x3.

for ii = 1:3
    data_structure.behavioral.summary.hgf_nu.traj(ii,:) = mean(reshape(data_structure.behavioral.hgf_nu.est.traj.muhat(:,ii),30,12),1);
end

[data_structure] = ch_hgf_calcx_nu(data_structure);
